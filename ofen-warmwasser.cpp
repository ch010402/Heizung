...
changed:
  ...
  ch010402 01.10.2022 make more verbose
*/
...
int main(int argc, const char** argv) {
  ...
  if (winterbetrieb) {
    ...
    while (true)
    {
      ...
      if (bu < 70) {
        ...
        Log::Info("Ofen Ruecklauf " + to_string(orl) + " °C - Boiler unten " + to_string(bu) + "°C");
        if (orl - 5 > bu) {
          boilervalve.open();
          boilerpumpe.on();
        }
  ...
  // sommerbetrieb
    ...
    while (true) {
      ...
      if (status >= elektroStatus::elektroStart && elektroRuecklauf.temperatur() < 44.0) {
        boilerpumpe.off();
        Log::Info("Schalte Boilerpumpe aus");
        boilervalve.close();
        Log::Info("Schliesse Boilerventil");
